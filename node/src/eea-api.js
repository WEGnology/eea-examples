...
const buildEEAImports = ({ deviceId, wasmMemory, onMessage, persistedStoragePath, bufferInfo }) => {
  return {
    /**
     * Invoked by the EEA to obtain the current Wegnology device ID. This is invoked as
    ...
    /**
     * Invoked by the EEA whenever a message needs to be sent to the Wegnology platform. This is
     ...
     * connected to Wegnology’s MQTT broker, these messages can be directly published, as-is,
...
const buildEEAExports = ({ wasmMemory, wasmInstance, bufferInfo }) => {
  return {
     ...
     * is primarily used to set the isConnectedToWegnology field that is automatically added
